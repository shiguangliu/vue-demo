...
export default {
    ...
    return {
      ...
      filterInfo: {
        ...
        list: [
          ...
          { type: 'input', label: '用户ID', value: 'soldUserId' },
          { type: 'input', label: '用户手机号', value: 'soldUserPhone' },
          { type: 'datetimerange', label: '出售时间', dateType: 'daterange', value: 'buyTimeDateRange', event: 'pickDate' },
          ...
          { type: 'customselect', label: '装备等级From', value: 'itemLevelFrom', list: 'itemLevelList', key: 'itemValue', name: 'itemName' },
          { type: 'customselect', label: '装备等级To', value: 'itemLevelTo', list: 'itemLevelList', key: 'itemValue', name: 'itemName' },
      ...
      tableInfo: {
        ...
        fieldList: [
          ...
          { label: '装备ID', value: 'itemId', width: 70 },
          { label: '装备名称', value: 'itemName' },
          { label: '装备类型', value: 'itemTypeStr', width: 70 },
          { label: '图片', value: 'imageThumbnailUrl', type: 'image' },
          { label: '装备等级', value: 'itemLevel', width: 70 },
          { label: '稀有度', value: 'rareTypeStr', width: 70 },
          { label: '是否赞助', value: 'sponsorFlagStr', width: 70 },
          { label: '货币类型', value: 'priceTypeStr', width: 70 },
          { label: '出售金额', value: 'salesPrice', width: 70 },
          { label: '数量', value: 'quantity', width: 50 },
          { label: '出售用户ID', value: 'soldUserId', width: 90 },
          { label: '出售用户手机号', value: 'soldUserPhone', width: 110 },
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      // this.$initDataPermits('skuPlatBuyInList', this.tableInfo.handle.btList) // 列表数据操作权限
      this.$initDataPermits('skuPlatBuyInList', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
      // form验证
    ...
    getItemLevelList() {
      const params = { 'configType': 'GAME_ITEM_LEVEL' }
      this.$handleAPI('', getConfigItemsListApi, params).then(res => {
        if (res.code === 200) {
