              ...
              >
                <template v-slot:form-fileLink>
                  <div class="slot-video">
                    <img
                      v-if="formInfo2.data.fileType === 1"
                      v-imgAlart
...
export default {
  ...
  data() {
    ...
    const checkGold = (rule, value, callback) => {
      const check = this.$validate({ label: '发放金币数量', value, rules: ['notnull', 'number', 'length'], conditions: [1, 11] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    return {
      ...
      formInfo: {
        ...
        fieldList: [
          { label: '动效ID', value: 'id', type: 'tag', required: true, maxlength: 200, className: 'el-form-block' },
          { label: '动效名称', value: 'title', type: 'input', required: true, maxlength: 200 },
          { label: '所属商户', value: 'merchantId', type: 'customselect', list: 'merchantList', key: 'id', name: 'merchantName', required: true, className: 'el-form-block' },
          ...
          { label: '投放限制', value: 'limitType', type: 'customradio', list: 'limitTypeList', key: 'itemValue', name: 'itemName', event: 'changeLimitType', className: '' },
          { label: '限制数值', value: 'maxTimes', type: 'input', validator: checkMaxTimes },
          { label: '发放方式', value: 'issueType', type: 'customradio', list: 'issueTypeList', key: 'itemValue', name: 'itemName', event: 'changeIssueType', className: '' },
          { label: '发放金币数量', value: 'gold', type: 'input', validator: checkGold },
      ...
      formInfo2: {
        ...
        fieldList: [
          ...
          { label: '提示弹窗图片', value: 'imageUrl', type: 'slot', className: 'el-form-block', required: true }
        ],
        rules: {},
        labelWidth: '120px'
      },
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('adPopDetail', this.filterInfo.list) // 条件区域操作权限
    },
    initList() {
    //  下拉框需要初始化加载的列表数据
      ...
      this.$handleAPI('', getConfigItemsListApi, { 'configType': 'AD_GOLD_ISSUE_TYPE' }).then(res => {
        if (res.code === 200) {
      ...
      this.$handleAPI('', getInfoApi, param).then(res => {
        if (res.code === 200) {
          ...
          this.formInfo.fieldList.forEach(element => {
            if (element.value === 'limitType') {
              if (this.formInfo.data.limitType === '1') {
                element.className = ''
              } else {
                element.className = 'el-form-block'
              }
            }
            ...
            if (element.value === 'issueType') {
              if (this.formInfo.data.issueType === '0') {
                element.className = 'el-form-block'
              } else {
                element.className = ''
              }
        ...
        case 'upload': { // 上传成功，绑定图片链接到id
          if (this.dialogUpload.type) {
            if (this.dialogUpload.type === 'uploadFile') {
              this.formInfo2.data.imageUrl = data.fileList[0].relativeUrl
              // 关闭弹窗
