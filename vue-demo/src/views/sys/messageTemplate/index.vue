  ...
  <div class="app-container">
    ...
    >
      ...
      <Upload
        ...
        :upload-data="{targetpath: 'goodsbrand',autoname: '1'}"
        @handleEvent="handleEvent"
      />
    </page-dialog>
  </div>
...
import { getListApi, createApi, updateApi, deleteApi } from '@/api/sys/messageTemplate/messageTemplate'
...
export default {
  ...
  data() {
    ...
    const checkID = (rule, value, callback) => {
      const check = this.$validate({ label: '消息模板ID', value, rules: ['notnull', 'noChinese', 'length'], conditions: [1, 11] })
      if (!check.result) {
    ...
    const checkContent = (rule, value, callback) => {
      const check = this.$validate({ label: '消息模板内容', value, rules: ['notnull', 'length'], conditions: [1, 500] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    // 检测消息编码
    const checkmessageCodeName = (rule, value, callback) => {
      const check = this.$validate({ label: '消息编码', value, rules: ['notnull', 'length'], conditions: [1, 50] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    return {
      ...
      filterInfo: {
        ...
        list: [
          { type: 'input', label: '消息编码', value: 'messageCode' },
          { type: 'input', label: '消息内容', value: 'content' },
      ...
      tableInfo: {
        ...
        data: [],
        fieldList: [
          { label: '消息模板ID', value: 'id', width: 90 },
          { label: '消息编码', value: 'messageCode', width: 300 },
          { label: '消息内容', value: 'content' },
          { label: '备注说明', value: 'remarks' },
      ...
      formInfo: {
        ...
        fieldList: [
          { label: '消息模板ID', value: 'id', type: 'tag', className: 'el-form-block-50', required: true, validator: checkID },
          { label: '消息编码', value: 'messageCode', type: 'input', className: 'el-form-block-50', required: true, validator: checkmessageCodeName },
          { label: '消息内容', value: 'content', type: 'input', className: 'el-form-block-100', required: true, validator: checkContent },
          { label: '备注说明', value: 'remarks', type: 'input', className: 'el-form-block-100', validator: checkRemarks },
    ...
    'dialogInfo.type'(val) {
      ...
      switch (val) {
        case 'create': {
          ...
          const params = { 'createBy': this.$store.state.staff.staffId }
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('sysMessageTemplate', this.tableInfo.handle.btList) // 列表数据操作权限
      this.$initDataPermits('sysMessageTemplate', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
      const formInfo = this.formInfo
    ...
    handleClick(event, data) {
      ...
      switch (event) {
        ...
        case 'delete': { // 删除
          ...
          this.$handleAPI(event, deleteApi, rowdata).then(res => {
