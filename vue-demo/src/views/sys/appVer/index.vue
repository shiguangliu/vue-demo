...
import { getListApi, createApi } from '@/api/system/appversion'
...
export default {
  ...
  data() {
    return {
      ...
      filterInfo: {
        ...
        list: [
          { type: 'customselect', label: '适用APP', value: 'appType', list: 'appTypeList', key: 'itemValue', name: 'itemName' },
          { type: 'input', label: '版本号', value: 'appVer' },
          { type: 'customselect', label: '状态', value: 'status', list: 'statusList', key: 'itemValue', name: 'itemName' },
          { type: 'customselect', label: '强制更新', value: 'forceFlag', list: 'forceList', key: 'itemValue', name: 'itemName' },
      ...
      tableInfo: {
        ...
        data: [],
        fieldList: [
          { label: '版本ID', value: 'id', width: 90 },
          { label: '适用APP', value: 'appTypeStr', type: 'tag' },
          { label: '版本号', value: 'appVer', width: 90 },
          { label: '下载链接', value: 'apkLink' },
          { label: '描述', value: 'description' },
          { label: '状态', value: 'statusStr', type: 'tag', width: 90 },
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('appVerList', this.tableInfo.handle.btList) // 列表数据操作权限
      this.$initDataPermits('appVerList', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
    },
