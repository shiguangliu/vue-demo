<template>
  <div class="app-container">
    ...
    <page-table
      ...
      @handleEvent="handleEvent"
    >
      <!-- <template v-slot:col-showDate="scope">
        <span>{{ scope.row.validFrom }} 至 {{ scope.row.validTo }}</span>
      </template>
      <template v-slot:col-showTime="scope">
        <span>{{ scope.row.timeFrom }}-{{ scope.row.timeTo }}</span>
      </template> -->
    </page-table>
  </div>
</template>
...
import { getListApi, createApi, deleteBatchApi } from '@/api/trip/tripmap'
  ...
  data() {
    return {
      ...
      filterInfo: {
        ...
        list: [
          { type: 'input', label: '顶部背景ID', value: 'id' },
          { type: 'input', label: '顶部背景名称', value: 'mapName' },
          { type: 'customselect', label: '状态', value: 'status', list: 'statusList', key: 'itemValue', name: 'itemName' },
          { type: 'datetimerange', label: '创建时间', value: 'startEndTime', dateType: 'daterange', event: 'pickDate' },
      ...
      tableInfo: {
        ...
        data: [],
        fieldList: [
          { label: '顶部背景ID', value: 'id', width: 90 },
          { label: '顶部背景名称', value: 'mapName' },
          { label: '展示日期', value: 'showDate' },
          { label: '展示时间', value: 'showTime', width: 90 },
          { label: '背景图片', value: 'wallUrl', type: 'image' },
          { label: '道路图片', value: 'roadUrl', type: 'image' },
        ...
        case 'list': // 对表格获取到的数据做处理
          if (data) {
            data.forEach(item => {
              ...
              if (item.validFrom !== '0000-00-00 00:00:00' && item.validFrom !== null && item.validTo !== '0000-00-00 00:00:00' && item.validTo !== null) {
                item.showDate = this.$fn.switchTime(item.validFrom, 'YYYY-MM-DD') + ' 至 ' + this.$fn.switchTime(item.validTo, 'YYYY-MM-DD')
              }
              if (item.timeFrom !== null && item.timeFrom !== ' ' && item.timeTo !== null && item.timeTo !== '') {
                item.showTime = item.timeFrom + '~' + item.timeTo
              }
            })
          }
          break
