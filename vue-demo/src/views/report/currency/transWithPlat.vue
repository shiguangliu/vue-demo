...
import { getReportCurrencyTransWithPlat } from '@/api/report/currency'
...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      this.tableHeight = window.innerHeight - this.$refs['table-transWithPlat'].$el.offsetTop - 150
    })
    this.initList()
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('reportCurrencyTransWithPlat', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
    },
    ...
    getList() {
      this.loading = true
      this.$handleAPI('', getReportCurrencyTransWithPlat, this.filterInfo.query).then(
        res => {
          if (res.code === 200) {
            this.tableData = res.data.currencyTransWithPlatList
            // 优化导出的格式
            this.editTable()
          }
          this.loading = false
    ...
    editTable() {
      var current = this.$refs['table-transWithPlat'].$el
    ...
    handleDownload(excelName) {
      try {
        const $e = this.$refs['table-transWithPlat'].$el
