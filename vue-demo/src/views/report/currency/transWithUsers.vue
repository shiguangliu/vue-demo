...
import { getReportCurrencyTransWithUsers } from '@/api/report/currency'
...
export default {
  ...
  mounted() {
    this.$nextTick(function() {
      this.tableHeight = window.innerHeight - this.$refs['table-transWithUsers'].$el.offsetTop - 150
    })
    this.initList()
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('reportCurrencyTransWithUsers', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
    },
    ...
    // 获取列表
    getList() {
      this.$handleAPI('', getReportCurrencyTransWithUsers, this.filterInfo.query).then(
        res => {
          this.loading = true
          if (res.code === 200) {
            this.tableData = res.data.currencyTransWithUsersList
            // 优化导出的格式
            this.editTable()
          }
          this.loading = false
    ...
    editTable() {
      var current = this.$refs['table-transWithUsers'].$el
    ...
    handleDownload(excelName) {
      try {
        const $e = this.$refs['table-transWithUsers'].$el
