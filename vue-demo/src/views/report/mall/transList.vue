...
import { getReportMallTransListApi as getListApi } from '@/api/report/mall'
    ...
    return {
      ...
      filterInfo: {
        ...
        list: [
          ...
          { type: 'customselect', label: '统计区分', value: 'reportType', list: 'statisTypeList', key: 'itemValue', name: 'itemName' },
          { type: 'customselect', label: '装备稀有度', value: 'rareType', list: 'rareTypeList', key: 'itemValue', name: 'itemName' },
          { type: 'customselect', label: '装备等级', value: 'itemLevel', list: 'levelList', key: 'itemValue', name: 'itemName' },
          { type: 'input', label: '礼包分类', value: 'setName' },
      ...
      tableInfo: {
        ...
        checkBox: false,
        showSummary: true,
        summaryFieldList: ['carTotal', 'carBuyTotal', 'partTotal', 'partBuyTotal', 'setTotal', 'setBuyTotal'],
        data: [],
        fieldList: [
          { label: '统计日期', value: 'reportDate' },
          { label: '统计区分', value: 'statisName' },
          { label: '车辆', value: '', labelList: [{ label: '数量', value: 'carTotal', align: 'right' }, { label: '人数', value: 'carBuyTotal', align: 'right' }] },
          { label: '配件', value: '', labelList: [{ label: '数量', value: 'partTotal', align: 'right' }, { label: '人数', value: 'partBuyTotal', align: 'right' }] },
          { label: '特惠礼包', value: '', labelList: [{ label: '数量', value: 'setTotal', align: 'right' }, { label: '人数', value: 'setBuyTotal', align: 'right' }] }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleDownload(list) {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        var merges = ['A1:A2', 'B1:B2', 'C1:D1', 'E1:F1', 'G1:H1']
        const multiHeader = [['统计日期', '统计区分', '车辆', '', '配件', '', '特惠礼包', '']]
        const tHeader = ['', '', '数量', '人数', '数量', '人数', '数量', '人数']
        var filterVal = ['reportDate', 'statisName', 'carTotal', 'carBuyTotal', 'partTotal', 'partBuyTotal', 'setTotal', 'setBuyTotal']
        ...
        excel.export_json_to_excel({
          ...
          merges,
          data,
          filename: '商城交易报表' + new Date().getTime(),
          autoWidth: true,
          bookType: 'xlsx'
        })
