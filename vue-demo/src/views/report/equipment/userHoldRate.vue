...
import { getReportEquipmentUserHoldRate } from '@/api/report/equipment'
...
export default {
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('reportEquipmentUserHoldRate', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
    },
    ...
    getList() {
      ...
      this.loading = true
      this.$handleAPI('', getReportEquipmentUserHoldRate, this.filterInfo.query).then(
        res => {
          if (res.code === 200) {
            res.data.equipmentRateDtos[0].reportDate = this.filterInfo.query.reportDate
            this.result = res.data
            this.tableData = res.data.equipmentRateDtos
            // 优化导出的格式
            this.editTable()
          }
          this.loading = false
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (index === 4) {
          sums[index] = this.result.avgTotalCarRate
          return
        }
        if (index === 7) {
          sums[index] = this.result.avgTotalPartRate
          return
        }
        const values = data.map(item => item[column.property])
        var countRows = 0
        const point = values.reduce((prev, curr) => {
          if (curr === undefined || curr === null || curr === '0.00%') {
            return prev
          }
          const value = parseFloat(curr) * 0.01
          if (!isNaN(value)) {
        ...
        sums[index] = Number(point * 100).toFixed(2) + '%'
        // if (index === 4 || index === 7 ) {
        //   if (sums[index - 2] > 0) {
