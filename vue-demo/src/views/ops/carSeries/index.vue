  ...
  <div class="app-container">
    ...
    >
      ...
      <Upload
        ...
        :upload-data="{targetPath: 'car-series',autoname: '1'}"
        @handleEvent="handleEvent"
      />
    </page-dialog>
  </div>
...
import { getListApi, createApi, updateApi, deleteApi } from '@/api/merchant/carSeries'
import { getConfigItemsListApi, getBrandGroupApi, getBrandNameListApi } from '@/api/common'
...
export default {
  ...
  data() {
    ...
    const checkID = (rule, value, callback) => {
      const check = this.$validate({ label: '系列ID', value, rules: ['notnull', 'noChinese', 'length'], conditions: [1, 11] })
      if (!check.result) {
    ...
    // 检测系列名称
    const checkSeriesName = (rule, value, callback) => {
      const check = this.$validate({ label: '系列名称', value, rules: ['notnull', 'length'], conditions: [1, 100] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    const checkFirstLetter = (rule, value, callback) => {
      const check = this.$validate({ label: '首字母', value, rules: ['notnull', 'noChinese', 'length'], conditions: [1, 10] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    return {
      ...
      tableInfo: {
        ...
        data: [],
        fieldList: [
          { label: '车辆系列ID', value: 'id', width: 90 },
          { label: '车辆系列名称', value: 'seriesName', width: 150 },
          { label: '首字母', value: 'firstLetter', width: 90 },
          { label: '所属品牌组', value: 'carGroupName', width: 200 },
          { label: '所属品牌', value: 'brandName', width: 200 },
          { label: '图片', value: 'icon', type: 'image', width: 200 },
          { label: '备注', value: 'remarks' },
          { label: '状态', value: 'itemName', type: 'tag', width: 90 },
      ...
      formInfo: {
        ...
        fieldList: [
          { label: '系列ID', value: 'id', type: 'tag', required: true, validator: checkID },
          { label: '系列名称', value: 'seriesName', type: 'input', required: true, validator: checkSeriesName },
          { label: '首字母', value: 'firstLetter', type: 'input', required: true, validator: checkFirstLetter },
          { label: '所属品牌组', value: 'groupId', type: 'customselect', list: 'brandGroupList', key: 'id', name: 'groupName', event: 'detailgroupId', required: true },
          { label: '所属品牌', value: 'brandId', type: 'customselect', list: 'carBrandNameList', key: 'id', name: 'brandName', required: true },
          { label: '图片', value: 'icon', type: 'slot', className: 'el-form-block' },
          { label: '备注', value: 'remarks', type: 'textarea', className: 'el-form-block', validator: checkRemarks },
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('carSeriesList', this.tableInfo.handle.btList) // 列表数据操作权限
      this.$initDataPermits('carSeriesList', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
      const formInfo = this.formInfo
    ...
    handleEvent(event, data) {
      switch (event) {
        ...
        case 'changegroupId': // 对选择车辆品牌组时做选择
          this.filterInfo.query.brandId = null
          this.listTypeInfo.carBrandNameList = null
          if (this.filterInfo.query.groupId != null && this.filterInfo.query.groupId !== '') {
            const params = {
              'groupId': this.filterInfo.query.groupId
            }
            this.$handleAPI('', getBrandNameListApi, params).then(res => {
              if (res.code === 200) {
                this.listTypeInfo.carBrandNameList = res.data
          ...
          break
        case 'detailgroupId': // 对选择车辆品牌组时做选择
          this.formInfo.data.brandId = null
          this.listTypeInfo.carBrandNameList = null
          console.log(this.formInfo.data)
          if (this.formInfo.data.groupId != null && this.formInfo.data.groupId !== '') {
            const params = {
              'groupId': this.formInfo.data.groupId
            }
            this.$handleAPI('', getBrandNameListApi, params).then(res => {
              if (res.code === 200) {
                this.listTypeInfo.carBrandNameList = res.data
