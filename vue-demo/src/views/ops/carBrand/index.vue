...
import { getListApi, createApi, updateApi, deleteApi } from '@/api/merchant/carBrand'
import { getConfigItemsListApi, getBrandGroupApi } from '@/api/common'
...
export default {
  ...
  data() {
    ...
    const checkID = (rule, value, callback) => {
      const check = this.$validate({ label: '品牌ID', value, rules: ['notnull', 'noChinese', 'length'], conditions: [1, 11] })
      if (!check.result) {
    ...
    const checkGoodsBrandName = (rule, value, callback) => {
      const check = this.$validate({ label: '品牌名称', value, rules: ['notnull', 'length'], conditions: [1, 100] })
      if (!check.result) {
        callback(new Error(check.message))
      } else {
        callback()
    ...
    return {
      ...
      filterInfo: {
        ...
        list: [
          { type: 'input', label: '品牌ID', value: 'id' },
          { type: 'input', label: '品牌名称', value: 'brandName' },
          { type: 'input', label: '首字母', value: 'firstLetter' },
          { type: 'customselect', label: '所属品牌组', value: 'groupId', list: 'brandGroupList', key: 'id', name: 'groupName' },
      ...
      tableInfo: {
        ...
        fieldList: [
          ...
          { label: '品牌名称', value: 'brandName', width: 200 },
      ...
      formInfo: {
        ...
        fieldList: [
          { label: '品牌ID', value: 'id', type: 'tag', required: true, validator: checkID },
          { label: '品牌名称', value: 'brandName', type: 'input', required: true, validator: checkGoodsBrandName },
          { label: '首字母', value: 'firstLetter', type: 'input', required: true, validator: checkFirstLetter },
          { label: '所属车辆品牌组', value: 'groupId', type: 'customselect', list: 'brandGroupList', key: 'id', name: 'groupName', required: true },
  ...
  methods: {
    // 初始化数据权限
    initDataPermits() {
      this.$initDataPermits('carBrandList', this.tableInfo.handle.btList) // 列表数据操作权限
      this.$initDataPermits('carBrandList', this.filterInfo.list) // 条件区域操作权限
    },
    // 初始化验证
    initRules() {
      const formInfo = this.formInfo
