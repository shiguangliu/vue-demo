...
export default {
  ...
  methods: {
    ...
    checkAllSuccess() {
      return Object.keys(this.fileList).every(item => (this.fileList[item].response && this.fileList[item].response.data))
    },
    // 检查文件类型
    checkFileType(check) {
      let result = true
    ...
    uploadSuccess(res, file, fileList) {
      ...
      if (res.code === '200' || res.code === 200) {
        ...
        this.$message({
          showClose: true,
          message: file.name + '上传失败:' + res.message,
          type: 'error',
          duration: 3500
        })
        return
      ...
      fileList.forEach((item, index) => {
        if (item.response && item.response.data) {
          succArr.push(item.response.data)
        }
      })
      const successData = { code: res.code, fileList: succArr }
      this.$emit('handleEvent', this.onSuccess, successData)
    },
    // 文件上传失败
