...
// import imageFileOpen from '@/assets/image/file-open.png'
// import imageFileClose from '@/assets/image/file-close.png'
...
export default {
  ...
  watch: {
    ...
    defaultClicked(val) {
      if (!val.id) return
      const data = (this.loadType === 2) ? this.lazyInfo : this.loadInfo
  ...
  methods: {
    ...
    renderContent(h, { node, data, store }) {
      ...
      // <img src={node.expanded ? imageFileOpen : imageFileClose}></img>
    ...
    handleLoadNode(node, resolve) {
      ...
      if (params.type === 'url') {
        ...
        data = {}
        params.data.forEach(item => {
          data[item.key] = item.default || item.value || node.data[item.key]
        })
        data[levelInfo.pKey] = node.data[levelInfo.key] || 0
      } else {
        console.log('没有传参数类型')
      }
        ...
        if (res.code === '200' || res.code === 200) {
          ...
          arr.forEach(item => {
            ...
            if (item.hasOwnProperty('isLeaf')) {
              item[treeProps.isLeaf] = Boolean(item['isLeaf'])
            } else {
              item[treeProps.isLeaf] = levelInfo.leaf
            }
          })
          // 得到数据后把数据给到父级，方便父级用到
          self.$emit('update:baseData', [...self.baseData, ...arr])
          // 设置默认高亮
          if (self.defaultHighLight || self.defaultHighLight === 0) {
            self.$nextTick(() => {
              self.$refs.TreeComponent.setCurrentKey(self.defaultHighLight)
            })
          }
          // 设置默认点击
          if ((self.defaultClicked && (self.defaultClicked.id || self.defaultClicked.id === 0))) {
            // 页面初始化，设置默认点击项， 并将点击事件派发到父级
            self.$emit('handleEvent', 'leftClick', { data: self.getSelectData(levelInfo.key, self.baseData, self.defaultClicked.id) || {}})
          }
        } else {
          if (res.message && res.message !== '') {
            self.$message({
