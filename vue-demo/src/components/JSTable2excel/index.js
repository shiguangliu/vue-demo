...
const table2excel = (column, data, excelName) => {
	...
	function getTextHtml(val) {
		...
		return `<td style="mso-number-format:'\@';text-align: center">${val}</td>`
	}
	...
	function getImageHtml(val, options) {
		if (val === null || val === undefined || val === '') {
			...
			return `<td style="width: ${options.width*1.3}px; height: ${options.height*1.3}px; text-align: center; vertical-align: middle"><img src="${val}" width=${options.width} height=${options.height} /></td>`
		}
	}
}
